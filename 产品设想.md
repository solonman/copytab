...
**2. 全局改写模式与`Tab`键补全逻辑:**
*   **后端补全请求处理流程:**
    1.  **第一优先级：精确匹配** `standard_info`表。
    2.  **第二优先级：语义匹配** `document_chunks`表。
*   **a. 当全局“改写模式”开启时:** 精确匹配返回原文；语义匹配调用LLM生成改写建议。
*   **b. 当全局“改写模式”关闭时:** 所有匹配都返回**原文**。语义匹配会额外返回一个标志位，用于前端显示“改写”浮动按钮。
